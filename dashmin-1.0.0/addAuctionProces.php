...
if (empty($title)) {
    ...
    if ($modelHasBrand->num_rows == 0) {
        ...
        Database::iud("INSERT INTO `product`(`model_has_category_id`,`title`,`description`,`product_status`,`uniq_id`,`added_datetime`,`expire_datetime`,`beginning_price`,`qty`) 
        VALUES('" . $modelHasCategoryID . "','" . $title . "','" . $description . "','" . $uid . "','1','" . $datetime . "','" . $exdate . "','" . $price . "','" . $qty . "')");

        $last_id = Database::$connection->insert_id;

        $itemId2 = Database::search("SELECT * FROM `auction` WHERE `uniq_id`='" . $uid . "' AND `title`='" . $title . "' ");
        ...
        Database::iud("INSERT INTO `product_item`(`auction_id`,`color_id`,`qty`,`price`,`date`) 
        VALUES('" . $last_id . "','" . $color . "','" . $qty . "','" . $price . "','" . $date . "')");
        ...
        Database::iud("INSERT INTO `auction_img`(`path1`,`auction_id`,`path2`,`path3`,`path4`,`path5`) VALUES('" . $fileName . "','" . $last_id . "','" . $fileName2 . "','" . $fileName3 . "','" . $fileName4 . "','" . $fileName5 . "' ) ");
    }
}
