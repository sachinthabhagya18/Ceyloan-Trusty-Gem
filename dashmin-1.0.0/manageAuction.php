...
<head>
    <meta charset="utf-8">
    <title>CTG Manage Auction</title>
...
<body>
    <div class="container-xxl position-relative bg-white d-flex p-0">
                ...
                <div class="navbar-nav w-75">
                    ...
                    <div class="nav-item dropdown">
                        ...
                        <div class="dropdown-menu bg-transparent border-0">
                            ...
                            <a href="manageAuction.php" class="dropdown-item active">Manage BID Product</a>
                        </div>
                    </div>
                    <div class="nav-item dropdown">
        ...
        <div class="content">
            ...
            $productrs = Database::search("SELECT * FROM `auction` ");
            ...
            $selectedrs = Database::search("SELECT  `auction`.`id` AS `auction_id`,`added_datetime`,`expire_datetime`,`auction_status_id`,`path1`,`path2`,`path3`,`path4`,`path5`,`title`,`beginning_price`,`description` FROM `auction`  INNER JOIN `product_item` ON `auction`.`product_item_id`=`product_item`.`id` 
            INNER JOIN `product` ON `product_item`.`product_id`=`product`.`id`
            INNER JOIN `product_img` ON `product_img`.`product_item_id`=`product_item`.`id`   LIMIT " . $result_per_page . " OFFSET " . $page_first_result . " ");
                        ...
                        <table class="table text-start align-middle table-bordered table-hover mb-0 text-center">
                            <thead>
                                <tr class="text-dark">
                                    ...
                                    <th scope="col">Begining Price</th>
                            ...
                            ?>
                                <tbody>
                                    <tr>
                                        ...
                                        <td><?php echo $srow["added_datetime"]; ?></td>
                                        <td><?php echo $srow["expire_datetime"]; ?></td>
                                        <td><?php echo $srow["beginning_price"]; ?></td>
                                        <td>
                                            <?php
                                            $s = $srow["auction_status_id"];
                                            if ($s == "1") {
                                            ?>
                                                <button class="btn btn-sm  btn-danger" onclick="blockproductau('<?php echo $srow['auction_id']; ?>');">Block</button>
                                            <?php
                                            } else {
                                            ?>
                                                <button class="btn btn-sm  btn-success" onclick="blockproductau('<?php echo $srow['auction_id']; ?>');">Unblock</button>
                                            <?php
                                            }
                                            ?>
