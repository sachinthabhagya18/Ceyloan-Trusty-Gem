...
if (!($c == "0") && ($s == "0") ) {  // category
    ...
    INNER JOIN `category` ON `model_has_category`.`category_id`=`category`.`id` INNER JOIN `product_item` ON `product`.`id`=`product_item`.`product_id` 
    INNER JOIN `product_img` ON `product_item`.`id`=`product_img`.`product_item_id` WHERE product.product_status_id='1' AND `category_id`='".$c."' ");

} elseif (!($s == "0") && ($c == "0")) { // search and age
    if ($s == 1) {
        $resultset = Database::search("SELECT * FROM `product` INNER JOIN `model_has_category` ON `product`.`model_has_category_id`=`model_has_category`.`id` 
        INNER JOIN `category` ON `model_has_category`.`category_id`=`category`.`id` INNER JOIN `product_item` ON `product`.`id`=`product_item`.`product_id` 
        INNER JOIN `product_img` ON `product_item`.`id`=`product_img`.`product_item_id` WHERE product.product_status_id='1'
         ORDER BY `price` ASC");
